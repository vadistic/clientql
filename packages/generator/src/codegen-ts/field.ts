import { isNotEmpty, isNullable } from '@graphql-clientgen/shared'
...
const codegenFieldArguments = (props: GeneratorProps) => (
  ...
  // print inline
  if (props.config.liftSingleArgument && node.arguments.length === 1) {
  ...
  node.arguments.forEach(inputValue => {
    result += '    ' + indent(codegenInputValue(props)(inputValue), 2) + '\n'
  })
