...
import { buildFragmentDoc } from '../src'
...
describe('prisma props > build fragment/selection', () => {
  it('prisma simple flat type', () => {
    ...
    expect(print(doc)).toMatchFile(...fileSnapPath('prisma.simple.graphql', 'fragment'))
  })

  it('prisma edge type', () => {
    const doc = buildFragmentDoc(prismaProps)('BoardEdge')!

    expect(print(doc)).toMatchFile(...fileSnapPath('prisma.edge.graphql', 'fragment'))
  })
})
...
describe('complex props > build fragment/selection', () => {
  it('complex props > selection on interface', () => {
    ...
    expect(print(doc)).toMatchFile(...fileSnapPath('complex.interface.graphql', 'fragment'))
  })
})
  ...
  it('no empty flat fragments are generated', () => {
    ...
    expect(
      doc.definitions.filter(
        node => node.kind === Kind.FRAGMENT_DEFINITION && node.selectionSet.selections.length === 0,
      ).length,
    ).toBe(0)

    expect(print(doc)).toMatchFile(...fileSnapPath('vats.no-empty.graphql', 'fragment'))
  })
})
